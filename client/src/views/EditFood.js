...
const AddFood = (props) =>{
    ...
    const [page_loading, setPagel_loading] = useState(false);

    useEffect(()=> {
        const {match:{params:{id}}} = props;
        const food = props.foods.filter(f => f._id === id)[0];
        if(id) setId(id);
        else props.history.push("/admin");
        setPrices(food.prices);
        setStandardPrice(food.price);
        setName(food.name);
    ...
    const handleUpdate = async() => {
        try{
            ...
            setLoading(true);
            await axios.post("/api/admin/foods/edit", 
            data, {
                timeout: 30000, 
                headers:{
            ...
            toast("Food updated successfully",  {style:{backgroundColor:"green", color:"#fff"}});
    ...
    return (
        ...
        />
                ...
                <Card className="card-user">
                    <CardHeader>
                        <CardTitle tag="h5">Edit Food</CardTitle>
                    </CardHeader>
                    <CardBody>
                    <Form>
                    <Row>
                        ...
                        <Col className="pl-1" md="3">
                            ...
                            <Input
                                onInput={t => {
                                    ...
                                    setVendorName(t.target.selectedOptions[0].textContent);
                                }}
